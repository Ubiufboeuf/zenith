...
export function DashboardMetrics () {
  ...
  return (
    <section class='h-fit w-full my-4 grid ml:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4'>
      { dashboardMetrics.map(({ label, value, trend, icon: MetricIcon, color }) => (
        <article
          key={`metric-card-${label.toLowerCase().replaceAll(' ', '-')}`}
          class='group grid grid-cols-[auto_1fr_auto] grid-rows-[1fr_auto] items-center justify-center gap-x-4 p-4 rounded-lg sm:rounded-xl border transition-colors bg-card border-border'
          style={{ '--color': color }}
        >
          <div class='row-span-full size-10 flex items-center justify-center text-(--color) bg-(--color)/8 rounded-lg'>
            <Icon class='size-6 stroke-2'>
              <MetricIcon />
            </Icon>
          </div>
          <h1 class='row-1 ml:sidebar-open:col-[2/4] text-xs text-neutral-400'>{label}</h1>
          <strong class='text-m font-bold'>{value}</strong>
          <div class='row-span-full ml:sidebar-open:row-2 ml:sidebar-open:col-3 flex gap-2 text-destructive'>
