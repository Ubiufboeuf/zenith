...
export function TabBarLink ({ id, href, title, icon: LinkIcon, onClick }: TabBarLink) {
  const activeItem = useTabBarStore((state) => state.activeItem)
  const isButtonActive = useTabBarStore((state) => state.isButtonActive)

  const active = activeItem === id && !isButtonActive
