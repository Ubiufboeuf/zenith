...
export function MobileResult ({ id, name, code, category, prices, priceId, stock }: Product) {
  ...
  return (
    <a href={`/product/${id}`} class={`${enoughStock ? 's' : 'ns'} group w-full h-fit grid grid-cols-[auto_1fr] grid-rows-[max-content_1fr_auto] items-center gap-3 p-4 grow-0 overflow-hidden rounded-xl border [.ns]:border-l-2 transition-colors border-border [.ns]:border-l-destructive text-light bg-card shr:border-primary/40`}>
